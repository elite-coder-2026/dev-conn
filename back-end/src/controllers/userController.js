...
async function createUser(req, res, next) {
  try {
    const { name, handle, avatar_url, cover_color, password_hash } = reconnection_queries.body
...
async function getMe(req, res, next) {
  try {
    const user = await userService.findById(reconnection_queries.user.id)
    res.json(user)
  } catch (err) { next(err) }
}
...
async function getById(req, res, next) {
  try {
    const user = await userService.findById(reconnection_queries.params.id)
    res.json(user)
  } catch (err) { next(err) }
}
...
async function searchUsers(req, res, next) {
  try {
    const query  = (reconnection_queries.query.q || '').trim()
    if (!query) {
...
async function updateMe(req, res, next) {
  try {
    const { name, avatar_url, cover_color } = reconnection_queries.body
    const user = await userService.updateMe(reconnection_queries.user.id, { name, avatar_url, cover_color })
    res.json(user)
  } catch (err) { next(err) }
}
...
async function deleteMe(req, res, next) {
  try {
    ...
    await userService.deleteUser(reconnection_queries.user.id)
    res.status(204).send()
  } catch (err) { next(err) }
}
